 ...
 *
 * 1. POST /auth/nonce   → server-generated nonce (single-use, TTL, stored in KV)
 * 2. wallet signs the SIWE message (frontend; any standard SIWE library)
 * 3. POST /auth/verify  → we parse the message with the reference ABNF parser
 *    (@spruceid/siwe-parser — no ethers dependency), enforce domain/chainId/
 *    freshness, recover the signer address with viem, and mint a session.
 *
