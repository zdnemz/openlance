...
metaRoutes.get('/ready', async (c) => {
  ...
  try {
    await getDb() // ensure the lazy singleton is initialised before raw SQL
  ...
  const ready = Object.values(checks).every((v) => v.startsWith('ok'))
  return ok(c, { ready, checks, mode: { db: env.databaseDriver, kv: env.queueMode === 'redis' ? 'redis' : 'in-process', chain: env.chainMode, storage: env.storageDriver } })
})

metaRoutes.get('/overview', async (c) => {
