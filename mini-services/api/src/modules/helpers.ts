...
export async function loadProject(projectId: string): Promise<Project> {
  const [p] = await (await getDb()).select().from(projects).where(eq(projects.id, projectId)).limit(1)
...
export async function loadMilestone(milestoneId: string): Promise<{ milestone: ProjectMilestone; project: Project }> {
  const [m] = await (await getDb()).select().from(projectMilestones).where(eq(projectMilestones.id, milestoneId)).limit(1)
...
export async function isParticipant(projectId: string, userId: string): Promise<boolean> {
  const rows = await (await getDb()).select({ id: projects.id })
    .from(projects)
